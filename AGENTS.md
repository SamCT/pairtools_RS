...
- When completing an implementation milestone, keep the completion commit and the next-active-milestone transition explicit in git history.
...
- Planned implementation milestones for remaining pairtools commands and parity expansions live in `milestones/M190` through `milestones/M260` after M161 real-data oracle validation is handled; do not start them until the active milestone is deliberately changed.
