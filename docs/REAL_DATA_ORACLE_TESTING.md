...
The available artifact checks compare parse stats against the pairtools stats file while allowing only the known `summary/complexity_naive` `nan`/`inf` representation difference. They also compare core dedup count fields where the currently scoped Rust stats can be interpreted against pairtools stats, and they compare duplicate-output readID routing between the available pairtools and pairs-rs duplicate files. The checks also print a split-compression observation when plain `rs_s01.outpairs.split.pairs` exists but `rs_s01.outpairs.split.pairs.gz` does not, matching the reported real-run behavior where `.gz` output failed but plain `.pairs` was produced. These checks are diagnostic only; canonical `merged.*` pairtools oracle outputs are still required before M161 can pass.
