...
usage() {
  cat <<'USAGE'
Run the exact hybrid Hi-C pipeline: pairs-rs replaces pairtools parse+sort only.

Required environment:
  THREADS       bwa-mem2 thread count
  SORT_THREADS  pairs-rs sort, pairtools merge, and samtools thread count
  ...
  TMPDIR        temporary directory for sort/merge
...
run_downstream() {
  ...
  if is_dry_run; then
    log "+ $(quote_cmd "${PAIRTOOLS_CMD[@]}" split --output-pairs merged.valid.pairs.gz --output-sam - merged.valid.pairsam.gz) | \\"
